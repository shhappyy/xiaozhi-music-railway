...
async function main() {
    ...
    xiaozhiWs.on('message', async (data) => {
          ...
          if (msg.method === 'initialize') {
                  ...
                  try { const result = await call(musicWs, 'tools/list', msg.params || {}); send(xiaozhiWs, { jsonrpc: '2.0', id: msg.id, result }); } catch (e) { send(xiaozhiWs, { jsonrpc: '2.0', id: msg.id, error: { code: -32603, message: String(e) } }); }
          } else if (msg.method === 'tools/call') {
                  try { const result = await call(musicWs, 'tools/call', msg.params || {}); send(xiaozhiWs, { jsonrpc: '2.0', id: msg.id, result }); } catch (e) { send(xiaozhiWs, { jsonrpc: '2.0', id: msg.id, error: { code: -32603, message: String(e) } }); }
          }
    });
