...
async function main() {
  ...
  // 小智 -> 音乐（拦截ping和initialize响应）
  xiaozhiWs.on('message', (data) => {
        const msg = data.toString();
        console.log('XIAOZHI ->:', msg.substring(0, 200));
        try {
                const parsed = JSON.parse(msg);
                // 拦截ping，自己回复，不转发给音乐MCP
          if (parsed.method === 'ping') {
                    console.log('Intercepted ping, replying directly');
                    if (parsed.id) {
                                send(xiaozhiWs, { jsonrpc: '2.0', id: parsed.id, result: {} });
                    }
                    return;
          }
